...
const animatedDiv = document.getElementById("animated_div"); // Додайте айді вашого <div> для анімації
...
const generateValue = (angleValue) => {
  for (let i of spinValues) {
    if (angleValue >= i.minDegree && angleValue <= i.maxDegree) {
      text.innerHTML = `<p>Вітаю! Тепер ти маєш витратити ці ${i.value}$ на бп Star Rail для всіх! </p>`;
...
// Створення аудіооб'єкту
const audio = new Audio('spin.mp3'); // Замініть 'sound.mp3' на URL або шлях до вашого аудіофайлу

spinBtn.addEventListener("click", () => {
  playSound();
  spinBtn.disabled = true;
  text.innerHTML = `<p>Що ж, яке число тобі попадеться!</p>`;
